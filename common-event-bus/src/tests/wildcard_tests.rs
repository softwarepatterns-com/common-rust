...
#[test]
fn wildstar_avoids_missing_words() {
  ...
  assert::equal(result, [].to_vec() as Vec<String>);
}

#[test]
fn wildstar_matches_missing_back() {
  let mut bus = Bus::<&str, _>::new();
...
#[test]
fn wildstar_matches_missing_front() {
  let mut bus = Bus::<&str, _>::new();
...
#[test]
fn wildstar_matches_single_missing_word() {
  let mut bus = Bus::<&str, _>::new();
